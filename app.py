...
# 1. Page Config & Disclosures (Q27.22)
...
def load_data():
    ...
    if not os.path.exists(file_path):
        st.error(f"Cannot find {file_path}. Please run reconcile_forecasts.py first.")
        return pd.DataFrame()
...
if not df.empty:
    ...
    # 4. Filter data for the plot
    plot_df = level_df[level_df['Entity'] == selected_entity].sort_values('d_num').dropna(subset=['snaive'])

    # --- IMPLEMENTATION Q27.19 & Q27.22: CALCULATE METRICS ---
    # Calculate RMSE for LightGBM vs Seasonal Naive
    ...
    # Calculate Empirical Interval Coverage (How often do actuals fall inside the 90% band?)
    # Target coverage is 90% (since we used q5 and q95)
    ...
    col3.metric("90% Interval Coverage", f"{coverage_pct:.1f}%", help="Target is ~90%. Shows the percentage of actual sales that fell within the p5-p95 confidence band.")
